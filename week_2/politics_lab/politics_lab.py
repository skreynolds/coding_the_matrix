...
def least_similar(sen, voting_dict):
    ...
    for e in voting_dict.keys():
        if e != sen:
            if policy_compare(sen,e,voting_dict) <= disagree_val:
                sen_disagree = e
                disagree_val = policy_compare(sen,e,voting_dict)
    return sen_disagree
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        -0.5
    """
    return sum([policy_compare(sen,e,voting_dict) \
                for e in sen_set if e != sen])/len(sen_set)

"""voting_dict = create_voting_dict()
listoflists = listoflists = [e.split() for e in voting_data]
democrat = {e[0] for e in listoflists if e[1] == "D"}
great_ave_sen = ""
great_ave_val = -46
for e in voting_dict.keys():
    if find_average_similarity(e,democrat,voting_dict) >= great_ave_val:
        great_ave_sen = e
        great_ave_val = find_average_similarity(e,democrat,voting_dict)
print(great_ave_sen)"""
...
def find_average_record(sen_set, voting_dict):
    ...
    return [sum(j)/len(sen_set) for j in zip(*[voting_dict[e] for e in sen_set])]
