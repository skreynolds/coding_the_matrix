...
def lin_comb_mat_vec_mult(M, v):
    ...
    return sum([v[k]*mat2coldict(M)[k] for k in v.D])
...
def lin_comb_vec_mat_mult(v, M):
    ...
    return sum([v[k]*mat2rowdict(M)[k] for k in v.D])
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    from matutil import mat2rowdict
    return Vec(M.D[0],{k:mat2rowdict(M)[k]*v for k in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    from matutil import mat2coldict
    return Vec(M.D[1], {k:v*mat2coldict(M)[k] for k in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    ...
    return coldict2mat({k:A*mat2coldict(B)[k] for k in B.D[1]})
...
def vM_mat_mat_mult(A, B):
    ...
    return rowdict2mat({k:mat2rowdict(A)[k]*B for k in A.D[0]})
...
def dot_prod_mat_mat_mult(A, B):
    ...
    from matutil import mat2rowdict
    from matutil import mat2coldict
    return Mat((A.D[0],B.D[1]),{(i,j):mat2rowdict(A)[i]*mat2coldict(B)[j] for i in A.D[0] for j in B.D[1]})
