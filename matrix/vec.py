...
def equal(u,v):
    ...
    res = True
    for d in u.D | v.D:
        if not getitem(u,d) == getitem(v,d):
            res = False
            break
    return res
...
def add(u,v):
    ...
    assert u.D == v.D
    return Vec(u.D|v.D,{key:(getitem(u,key)+getitem(v,key)) for key in u.D|v.D})

def dot(u,v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    return sum([getitem(u,key)*getitem(v,key) for key in u.D|v.D])

def scalar_mul(v, alpha):
    "Returns the scalar-vector product alpha times v"
    return Vec(v.D,{key:alpha*getitem(v,key) for key in v.D})
