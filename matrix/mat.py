...
def add(A, B):
    ...
    assert A.D == B.D
    return Mat(A.D,{(e,k):(getitem(A,(e,k))+getitem(B,(e,k))) for e in A.D[0] for k in A.D[1]})

def scalar_mul(M, alpha):
    "Returns the product of scalar alpha with M" 
    return Mat(M.D,{(e,k):alpha*getitem(M,(e,k)) for e in M.D[0] for k in M.D[1]})
...
def equal(A, B):
    ...
    for e in A.D[0]:
        for k in A.D[1]:
                if getitem(A,(e,k)) != getitem(B,(e,k)):
                    ind = False
                    break
    return ind
...
def matrix_matrix_mul(A, B):
    ...
    for i in Arow:
        for j in Bcol:
            res = Arow[i]*Bcol[j]
            if res != 0:
                dict_res.update({(i,j):res})
    return Mat((A.D[0],B.D[1]),dict_res)
